...
describe CeosAgency do
  describe "CEOS Agency" do
    it "is possible to create a CEOS Agency from a JSON string" do
      ...
      expect(c.archive_centers).to match_array(["AC1","AC2"])
  ...
  it "is possible to create two CEOS Agencies from a JSON string" do
    ...
    expect(first_agency.archive_centers).to match_array(["AC1","AC2"])
    expect(first_agency.data_centers).to match_array(["DC1", "DC2"])
    expect(second_agency.name).to eq('ceos_agency_2')
    expect(second_agency.archive_centers).to match_array(["AC3","AC4"])
  ...
  it "is possible to create a CMR query string from a JSON string" do
    ...
    expected_unescaped_query_string = 'archive_center[]=AC1&archive_center[]=AC2&archive_center[]=AC3&archive_center[]=AC4&data_center[]=DC1&data_center[]=DC2&data_center[]=DC3&data_center[]=DC4'
