...
describe "granules/mosdac" do
  it "is possible to create a collection-specific granule open search descriptor document with a valid client id" do
    ...
    render
    expect(rendered).to include("template=\"https://mosdac.gov.in/opensearch/datasets.atom?datasetId=3DIMG_L2B_HEM&amp;boundingBox={geo:box?}&amp;lat={geo:lat?}&amp;lon={geo:lon?}&amp;radius={geo:radius?}&amp;startTime={time:start?}&amp;endTime={time:end?}&amp;startIndex={startPage?}&amp;count={count?}&amp;gId={gId?}&amp;clientId=foo\">")
  ...
  it "is possible to create a collection-specific granule open search descriptor document without a valid client id" do
    ...
    render
    expect(rendered).to include("template=\"https://mosdac.gov.in/opensearch/datasets.atom?datasetId=3DIMG_L2B_HEM&amp;boundingBox={geo:box?}&amp;lat={geo:lat?}&amp;lon={geo:lon?}&amp;radius={geo:radius?}&amp;startTime={time:start?}&amp;endTime={time:end?}&amp;startIndex={startPage?}&amp;count={count?}&amp;gId={gId?}\">")
