...
describe "granules/descriptor_document" do
  it "is possible to create a granule open search descriptor document with a blank client id" do
      ...
      osdd_response_str = <<-eos
                  <os:OpenSearchDescription
                    ...
                    <os:Url type="application/atom+xml" rel="results"
                      params:method="GET"
                      template=\"#{ENV['opensearch_url']}/granules.atom?datasetId={echo:datasetId?}&amp;shortName=MOD02QKM&amp;versionId=005&amp;dataCenter=LAADS&amp;boundingBox={geo:box?}&amp;lat={geo:lat?}&amp;lon={geo:lon?}&amp;radius={geo:radius?}&amp;geometry={geo:geometry?}&amp;placeName={geo:name?}&amp;startTime={time:start?}&amp;endTime={time:end?}&amp;cursor={os:startPage?}&amp;numberOfResults={os:count?}&amp;offset={os:startIndex?}&amp;uid={geo:uid?}&amp;parentIdentifier={eo:parentIdentifier?}\">
...
describe "granules/descriptor_document" do
  it "is possible to create a granule open search descriptor document with no client id" do
